...
const MARK_LIST = `
   ...
   FROM
      semi_trailer_makes
   ORDER BY
      semi_trailer_make_name;
`;

const MODEL_LIST = `
   SELECT
      semi_trailer_model_id,
      semi_trailer_model_name
   FROM
      semi_trailer_models a
   INNER JOIN
      semi_trailer_makes b 
   ON
      a.semi_trailer_make_id = b.semi_trailer_make_id
   WHERE
      semi_trailer_make_name = $1
   ORDER BY
      semi_trailer_model_name;
`;
...
const UPDATE_MODEL = `
   UPDATE
      semi_trailer_models
   SET
      semi_trailer_make_id = $2,
      semi_trailer_model_name = $3
   WHERE
      semi_trailer_model_id = $1
   RETURNING *;
`;
