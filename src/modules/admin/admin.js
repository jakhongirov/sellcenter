...
module.exports = {
   GET_ADMIN: async (req, res) => {
      try {
         ...
         if (limit && offset) {
            const adminList = await model.adminList(limit, offset)
      ...
      try {
         ...
         if (checkAdminById) {
            if (password) {
               ...
               const editAdminPass = await model.editAdminPass(id, email, pass_hash)
               ...
               const editEmail = await model.editEmail(id, email)
